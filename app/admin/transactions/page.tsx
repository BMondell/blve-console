...
export default async function TransactionsPage() {
  ...
  const { data: transactions, error } = await supabase
    ...
    .select(`
      ...
      member_id,
      merchant_id,
      members!inner (name, email),  // join members
      merchants!inner (business_name, organization_id),  // join merchants
      merchants!inner (organizations!organization_id (name))  // join recruiting org name
    `)
  ...
  // Process: calculate commission (4% to recruiting org or BLVE default)
  const processed = transactions?.map((tx: any) => {
    const amount = tx.amount // assume dollars; if cents → amount / 100
          ...
          <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow-sm">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Recruiting Org (Gets Commission)</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {processed.map((tx: any) => (
                <tr key={tx.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{tx.merchants?.business_name || 'N/A'}</td>
